...
### 1.3 Scope and limitations
**Scope:** Web-based SPA for desktop and tablet; single-warehouse operations; weekly forecasts for stable-demand SKUs; user roles (Admin, Manager, Cashier); English localisation; printed or PDF receipts.
...
### 3.3 Web Application Development
- **Frontend technologies:** Vue 3 with Composition API was selected for reactive UI and manageable state. Alternatives (React, Angular) were considered; Vue’s gentle learning curve and single-file components suit rapid SME deployments.
...
Performance considerations included caching product lookups at the edge (HTTP caching for static assets, Redis/Laravel cache for catalog queries) and lazy-loading charts to avoid blocking POS operations. Frontend bundles are split by route using Vite’s code-splitting to keep the initial load under 250 KB on slow connections. Progressive enhancement ensures barcode scanners work as keyboard wedges without additional drivers.
...
### 3.6 Security Considerations
Secure coding followed OWASP guidelines: input validation with Laravel Form Requests, prepared statements via Eloquent, CSRF protection, and bcrypt password hashing. Sanctum secured SPA tokens; authorization policies enforced role-based access. Logs and audit trails capture critical actions (stock adjustments, refunds). HTTPS is required in deployment to protect credentials and tokens.
...
### 4.2 System Design and Architecture
The architecture is client–server with a Laravel REST API, MySQL database, and Vue SPA served via Vite. Authentication uses Sanctum tokens; caching layers (Laravel cache) reduce read latency for product catalogs and dashboards. Deployment targets a cloud VM with Nginx and PHP-FPM.
Services are decomposed into API, worker, and SPA containers for deployment portability. The API exposes REST endpoints secured by Sanctum and protected with Laravel throttling on sensitive surfaces (auth, analytics). The worker processes queued jobs such as forecast generation and email notifications; the SPA is a static bundle served by Nginx. This separation allows scaling the worker independently during forecast runs. Database migrations and seeders ensure deterministic setup in each environment (dev, staging, production). Observability hooks (HTTP access logs, Laravel Telescope in dev) provide insight into request patterns and slow queries. Configuration is 12-factor compliant with environment variables controlling database URIs, mail servers, cache drivers, and the optional Gemini API key used for AI summaries.
...
- **Prototyping:** Low-fidelity wireframes in Figma, then interactive Vue prototypes.
...
### 4.5 Summary
The chosen approach balances rapid delivery with rigor: Agile for adaptability, Laravel/Vue for productivity, C4 for clarity, and UCD for usability.
...
### 7.2 Container Diagram
The container model comprises a Vue SPA, Laravel API, MySQL database, optional worker queue for forecasts, and third-party services for email and authentication. Nginx serves static assets and proxies API traffic. (Figure 5 placeholder.)
...
Backend code uses PHP 8.x with Laravel; frontend uses TypeScript within Vue single-file components. PHP was chosen for Laravel’s ecosystem; TypeScript improves maintainability.

#### 7.2.4 Front End technology (50–70 words)
Vue 3, Vite, Pinia (state), Vue Router, Axios, and Chart.js compose the frontend. Component-based design accelerates reuse across product tables, POS cart, and dashboards.

#### 7.2.5 Back End technology (50–70 words)
Laravel provides routing, Eloquent ORM, validation, policies, queues, and Sanctum for SPA authentication. Job queues handle forecast generation to avoid blocking requests.
...
No native app is delivered; the responsive SPA supports tablets. Future Flutter or React Native wrappers could reuse the REST API for offline-focused use cases.
...
Operational safeguards include optimistic concurrency on stock adjustments to prevent race conditions during simultaneous checkouts. Idempotent webhook-style endpoints are designed for future payment gateway callbacks. Error handling follows a standard envelope (`code`, `message`, `errors`) consumed by the SPA to present actionable feedback. Logging uses contextual metadata (user_id, order_id) to trace issues. The overview also defines onboarding steps: upload product CSV, configure taxes and payment types, seed suppliers, then enable POS, ensuring a smooth go-live.
...
└─ views/
```
The scaffold keeps JavaScript lean: `app.js` is the entry point for Vite builds, and `bootstrap.js` configures Axios defaults (base URL, JSON headers, XSRF token) plus Echo placeholders for websocket features if introduced later.【F:resources/js/bootstrap.js†L1-L29】 This keeps the bundle minimal while ensuring HTTP clients are hardened for Sanctum-protected APIs.

#### 8.3.2 Source code samples (300–500 words)
`bootstrap.js` registers Axios with sane defaults, mirrors Laravel’s CSRF cookie handling, and leaves hooks for broadcasting so future UI components can stream updates without code changes to API clients.【F:resources/js/bootstrap.js†L1-L29】 `app.js` currently imports this setup and would host Vue component mounting when the UI is expanded. This pragmatic baseline allows the team to prioritise API completeness and testing while preserving a clear path to a richer SPA.
...
- **Validation and middleware:** Rate limits differentiate public auth, analytics, and AI routes, while Sanctum authentication and role middleware enforce RBAC for sensitive actions.【F:routes/api.php†L23-L132】 CORS configuration restricts origins while allowing credentialed SPA access.【F:config/cors.php†L5-L34】 Form Requests can be added alongside these controllers to centralise validation rules.
...
### 8.7 Deployment
Deployment uses environment-specific `.env` files, Composer/NPM installs, migrations, and `php artisan config:cache`. Nginx serves the compiled SPA and proxies `/api` to PHP-FPM. Screenshots of successful deployment and dashboards should be included as evidence.
...
The project improved proficiency in Laravel controllers/middleware, Vue build tooling, and analytics validation. Sprint cadence and CI discipline reduced integration pain. Time estimation for analytics hardening was optimistic; tuning queries and caches required more iterations than planned. Future projects will allocate explicit spikes for research tasks and invest earlier in automated UI testing.
