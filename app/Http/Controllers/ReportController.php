...
class ReportController extends Controller
{
    ...
    public function posReport(Request $request)
    {
        ...
        foreach ($orders as $o) {
            ...
            if ($method === 'cash') {
                if ($cur === 'CZK') {
                    $summary['sum_czk_cash'] += $o->amount_tendered_czk;
                }
                if ($cur === 'EUR') {
                    $summary['sum_eur_cash'] += $o->amount_tendered_eur;
                }
            } elseif ($method === 'card') {
                if ($cur === 'CZK') {
                    $summary['sum_czk_card'] += $o->amount_tendered_czk;
                }
                if ($cur === 'EUR') {
                    $summary['sum_eur_card'] += $o->amount_tendered_eur;
                }
            } elseif ($method === 'transfer') {
                if ($cur === 'CZK') {
                    $summary['sum_czk_transfer'] += $o->amount_tendered_czk;
                }
                if ($cur === 'EUR') {
                    $summary['sum_eur_transfer'] += $o->amount_tendered_eur;
                }
            }
